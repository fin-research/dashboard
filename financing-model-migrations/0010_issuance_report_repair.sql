-- Preserve the same publication contract while storing the four current-model terms and hit rate.
ALTER TABLE financing_model.issuance_run
  ADD COLUMN product_scenarios jsonb NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE financing_model.issuance_validation
  ADD COLUMN hit_rate_5bp double precision CHECK (hit_rate_5bp BETWEEN 0 AND 1);
...
BEGIN
  IF payload->>'schema_version' IS DISTINCT FROM 'issuance-forecast-v1'
    ...
    OR (payload ? 'product_scenarios' AND jsonb_typeof(payload->'product_scenarios') IS DISTINCT FROM 'array')
  THEN RAISE EXCEPTION 'Invalid issuance forecast publication'; END IF;
  IF payload ? 'product_scenarios' THEN
    IF jsonb_array_length(payload->'product_scenarios') <>
       (CASE WHEN jsonb_array_length(payload->'forecast')=0 THEN 0 ELSE 4 END)
      OR EXISTS (
        SELECT 1 FROM jsonb_array_elements(payload->'product_scenarios') WITH ORDINALITY AS scenario(scenario_value,ordinal)
        WHERE COALESCE((scenario_value->>'tenor')::numeric,-1) <>
                (CASE WHEN ordinal IN (1,3) THEN 3 ELSE 5 END)
           OR COALESCE(scenario_value->>'bond_type','') <>
                (CASE WHEN ordinal<=2 THEN '证券公司债' ELSE '证券公司次级债' END)
           OR COALESCE(jsonb_typeof(scenario_value->'coupon_percent'),'missing') NOT IN ('number','null')
           OR COALESCE(jsonb_typeof(scenario_value->'own_observations'),'missing') <> 'number'
           OR COALESCE(jsonb_typeof(scenario_value->'peer_observations'),'missing') <> 'number'
      )
    THEN RAISE EXCEPTION 'Invalid issuance product scenarios'; END IF;
  END IF;
  ...
  row_value := jsonb_build_object('run_id',stored_id,'issuer',payload #> '{terms,issuer}','deadline',payload->'deadline',
    ...
    'prediction_end',payload #> '{validation,prediction_end}',
    'product_scenarios',COALESCE(payload->'product_scenarios','[]'::jsonb),
    'calendar_year',payload #> '{calendar_audit,calendar,year}',
