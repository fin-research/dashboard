...
运行入口：门户 `/`，资金日报 `/fund-report`，管理 `/management`，市场点评 `/market-briefing`，文字版 `/market-briefing/text`，市场热点 `/market-hotspots`，政策跟踪 `/policy-tracking`，新闻资讯 `/news/[id]`，研报详情 `/articles/[id]`，研究点评 `/commentaries/[id]`，融资择时模型 `/financing-model`，二级池 `/bond`。
...
- 市场点评由浏览器直接请求 Data Worker 的单一上游映射资源，在 `src/market-report-resources.ts` 加工为视觉版与文字版共享的完整契约；不得请求 `/data/market-report/*`，不得经过 Dashboard Worker 聚合，也不得恢复 Data GraphQL 市场报告查询。
