...
import { isCreditOriginalKey, isPublicCreditDocument } from "../src/lib/server/credit-evidence.ts";
...
const prepared = corpusSchema.parse(JSON.parse(await readFile(path.join(directory, "corpus.json"), "utf8")));
const documents = prepared.documents.filter(isPublicCreditDocument);
const ids = new Set(documents.map(document => document.id));
const corpus = { ...prepared, documents, blocks: prepared.blocks.filter(block => ids.has(block.documentId)),
  searchFiles: prepared.searchFiles?.filter(file => ids.has(file.documentId)) };
if (!documents.length || !corpus.searchFiles?.length || corpus.searchFiles.some(file => !file.key.startsWith("search/定期报告/"))) {
  throw new Error("公开检索目录无效");
}
const report = JSON.parse(await readFile(path.join(directory, "preparation-report.json"), "utf8"));
if (report.failures.some(failure => failure.file?.startsWith("定期报告/")) || corpus.blocks.some(block => block.extraction === "unreadable")) throw new Error("公开材料解析有失败或未识别页，请先修复");
...
const previousRaw = previousFile ? corpusSchema.parse(JSON.parse(await readFile(previousFile, "utf8"))) : null;
const previousIds = new Set(previousRaw?.documents.filter(isPublicCreditDocument).map(document => document.id));
const previous = previousRaw ? { ...previousRaw, documents: previousRaw.documents.filter(isPublicCreditDocument),
  searchFiles: previousRaw.searchFiles?.filter(file => previousIds.has(file.documentId)),
  blocks: previousRaw.blocks.filter(block => previousIds.has(block.documentId)) } : null;
...
const objectPath = key => "/r2/buckets/eastmoney/objects/credit/" + key.split("/").map(encodeURIComponent).join("/");
const summary = { mode: apply ? "apply" : "dry-run", documents: corpus.documents.length, markdownFiles: corpus.searchFiles.length,
  searchBytes: corpus.searchFiles.reduce((n, f) => n + f.bytes, 0), objects: objects.length, obsoleteObjects: obsolete.length, bucket: "eastmoney", prefix: "credit/" };
...
if (info.source !== "eastmoney" || info.type !== "r2") throw new Error("AI Search credit 未连接 eastmoney 存储桶");
...
const checkpointFile = path.join(directory, "upload-checkpoint-public-v1.json");
...
const catalog = Buffer.from(JSON.stringify(corpus));
...
const deletedFile = path.join(directory, "pruned-public-objects.json");
...
await jsonRequest(instancePath, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ paused: before.paused,
  source_params: { ...info.source_params, include_items: ["credit/search/**", "/credit/search/**"], exclude_items: ["credit/originals/**", "/credit/originals/**", "credit/catalog/**", "/credit/catalog/**"] }, chunk: true }) });
