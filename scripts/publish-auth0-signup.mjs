...
const [template, fields, registrationCode, profileCode] = await Promise.all([
  load('branding/page-template.html'), load('branding/signup-fields.html'),
  load('actions/eastmoney-registration.cjs'), load('actions/eastmoney-signup-profile.cjs'),
]);
assert.match(template, /auth0:head/); assert.match(template, /auth0:widget/);
if (fields.length > 10000) throw new Error('Signup partial exceeds Auth0 limits');
...
const partials = management('get', 'prompts/signup/partials');
let previousTemplate = null;
try { previousTemplate = management('get', 'branding/templates/universal-login'); }
catch (error) { if (error.status !== 404) throw error; }
const previousBody = typeof previousTemplate === 'string' ? previousTemplate : previousTemplate?.body;
if (previousBody && previousBody !== template) throw new Error('Review the existing custom page template before replacing it');
const guardedFields = `{% if application.id == '${clientId}' %}\n${fields}{% endif %}`;
if (partials.signup?.['form-content-end'] && partials.signup['form-content-end'] !== guardedFields) throw new Error('Preserve the existing signup form partial');
const backupDirectory = await mkdtemp(join(tmpdir(), 'eastmoney-auth0-signup-'));
await writeFile(join(backupDirectory, 'before.json'), JSON.stringify({ template: previousTemplate, partials,
...
if ((profile.secrets ?? []).some((item) => !['EASTMONEY_CLIENT_ID', 'PROFILE_CLIENT_ID', 'PROFILE_CLIENT_SECRET'].includes(item.name))) throw new Error('Review unrelated Action secrets before updating');
management('patch', `actions/actions/${profile.id}`, { code: profileCode, secrets: [
  { name: 'EASTMONEY_CLIENT_ID', value: clientId }, { name: 'PROFILE_CLIENT_ID', value: manager.client_id }, { name: 'PROFILE_CLIENT_SECRET', value: manager.client_secret }] });
...
management('put', 'branding/templates/universal-login', { template });
management('put', 'prompts/signup/partials', { ...partials, signup: { ...partials.signup, 'form-content-end': guardedFields } });
// Require both fields only after the form and root-profile synchronization exist.
...
assert.equal(management('get', 'prompts/signup/partials').signup['form-content-end'], guardedFields);
const publishedTemplate = management('get', 'branding/templates/universal-login');
assert.equal(typeof publishedTemplate === 'string' ? publishedTemplate : publishedTemplate.body, template);
...
console.log(JSON.stringify({ verified: true, registrationAction: registrationId, profileAction: profile.id, loginDomain: domain }));
