...
test('消息记录、尝试明细和不确定结果重试确认',async({page},testInfo)=>{
  ...
  await page.screenshot({path:testInfo.outputPath('messenger-before.png'),fullPage:true,animations:'disabled'});
  ...
  await page.screenshot({path:testInfo.outputPath('messenger.png'),fullPage:true,animations:'disabled'});
});
