...
function harness() {
  ...
  let stored=[];
  const candidate={ruleId:'r',periodId:'p',targetId:'t',ruleName:'提醒',recipientMode:'custom',recipients:['a@example.com'],leadHours:24,projectName:'项目',taskName:'申报',sopName:'公司债',nodeName:'申报',debtType:'公司债',triggerDate:'2026-09-18',scheduledFor:'2026-09-17T01:00:00Z'};
...
test('融资入队保存中台ID而非已发送，下一轮不重复提交',async()=>{
  const h=harness(),options={...h,directory:async()=>[],asOf:'2026-09-17T02:00:00Z'};
...
test('融资提交失败可重试，dry-run不调用消息服务',async()=>{
  ...
  const options={...h,directory:async()=>[],asOf:'2026-09-17T02:00:00Z'};
