...
toggle(groups[0],'新增');assert.equal(visible().length,6);
toggle(groups[0],'到期');assert.equal(visible().length,7);
...
all(currentGroups[1]);assert.equal(visible().length,7);
all(currentGroups[0]);assert.equal(visible().length,10);
toggle(currentGroups[0],'续作/扩额');assert.equal(visible().length,7);
