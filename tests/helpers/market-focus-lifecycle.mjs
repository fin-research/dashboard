...
assert.equal(app.value(), '原始草稿');
assert.equal(document.querySelector('.focus-summary').textContent, '<b>摘要</b>');
assert.equal(document.querySelector('.focus-summary b'), null);
assert.equal(document.querySelector('[contenteditable]').hidden, true);
