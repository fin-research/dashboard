...
test('未知年度 GET 在交易日证据不可用时保持503，错误来源不误报为R2', async (context) => {
  ...
  const response = await GET({
    ...
    platform: { env: { EASTMONEY: bucket, DATA: { fetch: async () => new Response('destination_unavailable', { status: 503 }) } } },
  });
  assert.equal(response.status, 503);
  assert.deepEqual((await response.json()).error, {
    code: 'TRADING_CALENDAR_UNAVAILABLE', source: 'Dashboard Calendar', stage: 'report_date',
  });
