...
<div class="profile-page">
  <header class="profile-header">
    <Button data-ui-owner="routes-profile--page-svelte" variant="ghost" size="icon" class={"ui-button   profile-back"} href="/" aria-label="返回市场研究门户"><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m12.5 4-6 6 6 6" /></svg></Button>
    <h1>个人管理</h1>
  </header>
  <main class="profile-content">
    ...
    {:else if loadError}
      <div class="profile-state" role="alert"><p>{loadError}</p><Button data-ui-owner="routes-profile--page-svelte" variant="outline" class={"ui-button  profile-button"} type="button" onclick={() => loadProfile()}>重新读取</Button></div>
    {/if}
    <div class="profile-grid">
      <ModuleCard labelledBy="profile-details-title">
        ...
        <form class="profile-form" onsubmit={(event) => { event.preventDefault(); void submit('name'); }}>
          ...
          <div class="profile-actions"><Button data-ui-owner="routes-profile--page-svelte" variant="default" class={"ui-button  profile-button primary"} type="submit" disabled={!profile || pending !== null}>{pending === 'name' ? '正在保存' : '保存个人资料'}</Button></div>
        </form>
      </ModuleCard>
      <ModuleCard labelledBy="profile-email-title">
        <PanelHeading id="profile-email-title" title="登录邮箱" accent="var(--business-accent)" />
        <form class="profile-form" onsubmit={(event) => { event.preventDefault(); void submit('email'); }}>
          ...
          <Input data-ui-owner="routes-profile--page-svelte" class={"ui-input"} id="profile-email" name="email" type="email" autocomplete="email" bind:value={email} required maxlength={254} pattern="[^@\s]+@18\.[cC][nN]" disabled={!profile || pending !== null} />
          <label class="profile-choice"><Checkbox data-ui-owner="routes-profile--page-svelte"  bind:checked={confirmed} disabled={!profile || pending !== null} /><span>确认修改邮箱并重新验证</span></label>
          <div class="profile-actions"><Button data-ui-owner="routes-profile--page-svelte" variant="default" class={"ui-button  profile-button primary"} type="submit" disabled={!profile || pending !== null || !confirmed || email.trim().toLowerCase() === profile.email.toLowerCase()}>{pending === 'email' ? '正在更新' : '更新邮箱并退出登录'}</Button></div>
        </form>
      </ModuleCard>
      <ModuleCard labelledBy="profile-password-title">
        ...
        <div class="profile-form">
          ...
          <Input data-ui-owner="routes-profile--page-svelte" class={"ui-input"} id="password-recipient" value={profile?.email || data.email} readonly />
          <div class="profile-actions"><Button data-ui-owner="routes-profile--page-svelte" variant="outline" class={"ui-button  profile-button"} type="button" disabled={!profile || pending !== null} onclick={() => submit('password')}>{pending === 'password' ? '正在请求' : '发送密码重置邮件'}</Button></div>
        </div>
      </ModuleCard>
      <ModuleCard labelledBy="profile-preferences-title">
        ...
        <form class="profile-form" onsubmit={(event) => { event.preventDefault(); persistPreferences(); }}>
          ...
          <div class="profile-actions"><Button data-ui-owner="routes-profile--page-svelte" variant="default" class={"ui-button  profile-button primary"} type="submit">保存个性化配置</Button></div>
        </form>
      </ModuleCard>
        ...
        <ModuleCard labelledBy="profile-permissions-title">
          ...
          {#if profile}
            ...
            <div class="permission-footer">
              <Button data-ui-owner="routes-profile--page-svelte" variant="outline" class={"ui-button  profile-button"} href="/auth/login?returnTo=%2Fprofile"><RefreshCw size={18} aria-hidden="true" />刷新权限</Button>
            </div>
          {:else}<div class="permission-status" role="status">{#if loading}<span class="ui-spinner " aria-label="加载权限"></span>{:else}<Button data-ui-owner="routes-profile--page-svelte" variant="outline" class={"ui-button "} type="button" onclick={() => loadProfile()}>重新加载</Button>{/if}</div>{/if}
        </ModuleCard>
      </div>
    </div>
