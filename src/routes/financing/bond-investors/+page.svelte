...
<div class="investor-page">
  ...
  {:else}
    ...
    <InvestorChartTable id="investor-total-distribution" title="累计投资机构类型分布" empty={report.total[totalType] === 0} option={financingCompositionOption(totalDistribution.map(row => ({ type: row.name, amountYi: row.values[0] ?? 0 })))}
      ...
      {#snippet controls()}<label for="total-bond-type">债券品种</label><NativeSelect data-ui-owner="routes-financing-bond-investors--page-svelte" id="total-bond-type" class={"ui-select "} bind:value={totalType}><option value="total">全部品种</option>{#each investorBondTypes as type}<option value={type}>{type}</option>{/each}</NativeSelect>{/snippet}
    </InvestorChartTable>
    <InvestorChartTable id="investor-outstanding-distribution" title="存续投资机构类型分布" empty={report.outstanding[outstandingType] === 0} option={financingCompositionOption(outstandingDistribution.map(row => ({ type: row.name, amountYi: row.values[0] ?? 0 })))}
      ...
      {#snippet controls()}<label for="outstanding-bond-type">债券品种</label><NativeSelect data-ui-owner="routes-financing-bond-investors--page-svelte" id="outstanding-bond-type" class={"ui-select "} bind:value={outstandingType}><option value="total">全部品种</option>{#each investorBondTypes as type}<option value={type}>{type}</option>{/each}</NativeSelect>{/snippet}
    </InvestorChartTable>
    <InvestorChartTable id="investor-ranking" title="具体投资人情况" empty={ranking.length === 0} option={investorRankingOption(ranking)} scroll chartHeight={Math.max(27, ranking.length * 3.8 + 6)}
      ...
      {#snippet controls()}
        <label for="investor-bond-type">债券品种</label><NativeSelect data-ui-owner="routes-financing-bond-investors--page-svelte" id="investor-bond-type" class={"ui-select "} bind:value={investorType}><option value="total">全部品种</option>{#each investorBondTypes as type}<option value={type}>{type}</option>{/each}</NativeSelect>
        <label for="investor-sort">排序</label><NativeSelect data-ui-owner="routes-financing-bond-investors--page-svelte" id="investor-sort" class={"ui-select "} bind:value={sort}><option value="total">累计规模</option><option value="outstanding">存续规模</option></NativeSelect>
      {/snippet}
    </InvestorChartTable>
  {/if}
</div>
