  ...
  <div class="permission-workspace">
    <ModuleCard class="role-catalog">
      ...
      <ul class="role-list" aria-labelledby="role-catalog-title">
        {#each data.roles as item}<li><button class="btn btn-ghost" class:btn-active={selectedRole === item.id} type="button" aria-pressed={selectedRole === item.id} onclick={() => selectedRole = item.id}><span>{roleLabel(item.name)}</span><span class="badge badge-ghost">{data.configurations[item.id]?.permissions.length ?? 0}</span></button></li>{/each}
      </ul>
    </ModuleCard>
    <ModuleCard class="permission-editor">
      {#if role}<PanelHeading id="role-permissions-title" title={roleLabel(role.name)}><span class="badge badge-ghost">只读</span></PanelHeading><PermissionExplorer permissions={data.configurations[selectedRole]?.permissions ?? []} />{:else}<p>暂无角色。</p>{/if}
    </ModuleCard>
  </div>
</div>
<style>
