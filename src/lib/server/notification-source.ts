import { identityJson } from './gateway-client';
import { readWorkflowConfig } from './trading-workflow';
import { readProgress } from './trading-progress';
import { dueReminders, minutes, shanghaiClock } from '../trading-workflow/model';
import { messengerJson } from './messenger';
...
export async function notificationSource(request:Request,env:Env) {
 ...
 const users=await identityJson<{id:string;categories:string[]}[]>(env,'/directory/notification-users');
