...
- 收益与风险指标中的“收益率（含免税）”和“收益率（不含免税）”直接展示最新报表日 `daily_statistics` 的两个年化收益率源字段，不由前端重算。波动率和最大回撤只使用所选区间内业务本金大于零的有效交易日，单日收益率为“当日营收 ÷ 业务本金”：波动率按单日收益率样本标准差乘以 `√252`，最大回撤按区间复合净值的峰谷跌幅计算；有效交易日不足时不展示不可计算的指标。
