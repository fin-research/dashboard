...
- `PATCH /api/credit`：以 `(reportDate, institutionName, expectedUpdatedAt)` 定位一条机构记录，`changes.institution` 仅传发生变化的主体字段，`changes.items` 仅传发生变化的分项及字段；`updated_at` 以 UTC 微秒精度作为乐观并发版本，防止旧响应覆盖新数据。成功只返回变更机构及受影响的服务端派生汇总；版本确有冲突返回 409，空增量或非法字段返回 400。
