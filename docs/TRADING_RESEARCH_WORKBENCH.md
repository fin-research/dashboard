...
| 交易流程 | 三品种连线流程图、按需展开分支、画布拖动编辑、群价记录、浏览器到点提醒 | 后端进度、服务器定时推送、审批执行与聊天发送 |
...
- 交易流程使用 Svelte Flow，顶部固定排列拆借、逆回购、交易所逆回购白底品种标签，点击折叠或展开。群价分别位于拆借和逆回购下方，点击向下展开可编辑输入框，完成操作位于展开区域；按账号、上海自然日和原节点 ID 保存记录，旧记录兼容。
- 名称、内容、时间、分支结构和所属条件上下文一致的跨品种步骤按出现次序自动汇合；顺序相反会形成环时保持分开。编辑差异后自动拆分，群价不合并。合并仅为视图派生，保留全部源 ID；点击完成同时更新成员，编辑面板可选择单一来源节点。共享节点和自动汇合节点不受品种折叠影响；分支折叠仅隐藏自身子树。
- 未完成节点及品种标签统一白底；完成节点使用加深的原语义色，图标不变。分支全部后代任务（含折叠子分支）完成后才着色。顶部时钟每秒显示上海时间，时间轴全长蓝色，淡水平线按显式时点插值并从节点下方穿过。移除 Flow 标识。
- 编辑模式下直接拖动节点调整位置，点击节点打开右侧编辑面板；拖动不改变业务顺序及分支关系，无拖动按钮。偏移量相对自动布局保存于配置的可选 offset 字段，分支展开后仍跟随布局；面板数字字段提供键盘与单指操作替代。群价输入框可通过可选 inquiry 字段配置，既有询价节点兼容旧配置。分支及品种的编辑预览不修改当日激活状态，取消编辑不保存配置；保存失败保留草稿。
