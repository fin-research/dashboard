...
工作台当前包含七个标签页：总览、交易管理、授信管理、研究辅助、流程中心、二级池周报和融资择时模型。授信管理已经接入 Neon `credit` schema：业务人员每周在本地解析 Excel 并按报告日期写入数据库，浏览器通过同源 `/api/credit` 和 Hyperdrive 读取。研究辅助的首次全历史回填由受控本地命令完成；此后 dashboard Worker 每日增量读取 Choice EDB 与 DM 资金利率历史并写入 Neon `public.edb`，浏览器只通过同源 `/api/economic-indicators` 读取；交易和流程中心仍使用仓库内冻结数据；二级池与融资择时复用现有生产页面组件和各自既有数据链路。
