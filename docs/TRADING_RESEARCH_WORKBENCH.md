...
- 页面使用 Svelte Flow 呈现流程图，不使用大卡片包裹品种。共通节点位于启用品种的中轴，交易完成后节点汇合在流程底部；停用品种在右侧竖排折叠，点击启用并展开，再点击品种标题停用。普通节点整体点击切换完成状态，无内嵌操作按钮。询价节点右侧连接文本框，记录按账号及上海自然日保存在本地；旧版每日记录自动补空 notes，不丢失已有进度。
...
- 页头时间每秒按上海时间更新，节点只展示明确配置的开始/结束时间与到点待办状态，不为未定时节点增加期限，也不改变业务进度。画布支持平移、缩放与适应视图；动画尊重 reduced-motion，定位待办仅由按钮触发。
