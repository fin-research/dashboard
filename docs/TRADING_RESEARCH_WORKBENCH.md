...
- 交易流程使用 Svelte Flow，顶部拆借、逆回购、交易所回购为分类项，不作为节点、不连线，保留品种浅色底框，文字居中，不设归档按钮。右侧始终纵向排列三条流程的横文字标签，点击切换展开/折叠；剩余品种重排填满工作区，过渡为 240ms，减少动效时关闭。询价分别位于各品种下方，点击展开可编辑表格，不参与完成状态和待办提醒；按账号、上海自然日和原节点 ID 保存记录，旧记录兼容。
