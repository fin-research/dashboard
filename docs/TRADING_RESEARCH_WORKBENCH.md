...
- 交易流程使用 Svelte Flow，顶部拆借、逆回购、交易所回购为分类项，不作为节点、不连线，仅归档图标可折叠。折叠后在右侧形成纵向排列的竖排紧凑标签，点击展开；剩余品种重排填满工作区，过渡为 240ms，减少动效时关闭。询价分别位于各品种下方，点击展开可编辑表格，不参与完成状态和待办提醒；按账号、上海自然日和原节点 ID 保存记录，旧记录兼容。
- 拆借询价字段为对手方、交易员、期限、金额（亿）、价格；逆回购省略交易员，价格为实际利率。无表头、无按钮；Tab 选中候选并前移，末格 Tab/Enter 确认并进入下一行，Ctrl/Command+Backspace/Delete 删除当前记录，Shift+Tab 返回。输入即时保存在本地，期限、金额和价格留空沿用上一行；名单按账号跨日期维护，支持汉字、拼音首字母及部分匹配。旧 notes 内容保留为原询价记录，不强行结构化猜测。
...
- 名称、内容、时间、分支结构和所属条件上下文一致的跨品种步骤按出现次序自动汇合；顺序相反会形成环时保持分开。编辑差异后自动拆分，询价不合并。合并仅为视图派生，保留全部源 ID；点击完成同时更新成员，编辑面板可选择单一来源节点。共享节点和自动汇合节点不受品种折叠影响；分支折叠仅隐藏自身子树。
- 未完成节点及品种标签统一白底；完成节点使用加深的原语义色，图标不变。分支全部后代任务（含折叠子分支）完成后才着色。顶部时钟每秒显示上海时间，时间轴全长蓝色，淡水平线按显式时点插值并从节点下方穿过。移除 Flow 标识。
...
- 左侧时间线使用可见节点的显式起始时间作为锚点，时间游标每秒随上海时钟在锚点间插值；重复时间合并，非递增时点不作为坐标锚点，不给无时间节点创造期限。节点按正常字号与自然高度渲染，禁止 fitView 压缩整图；较长流程通过内容区自然滚动，窄屏仅画布局部横向滚动。节点实测高度变动后重排并更新连线。
