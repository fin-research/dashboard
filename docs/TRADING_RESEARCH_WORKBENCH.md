...
- 交易流程必须按用户确认的参考图组织版式：左侧蓝色时间线，紧凑白色节点，蓝/绿圆角直角连线，分流后底部汇合。不得自行改成缩略画布、卡片列表或设计另一套布局。页面使用 Svelte Flow 呈现流程图，不使用大卡片包裹品种。08:30 询价前置于 10:00 共通交易要素，不因 shared scope 排在产品任务之前而倒置时间顺序；两品种询价仍各自保留稳定 ID、进度和内容。共通节点位于启用品种的中轴，交易完成后节点汇合在流程底部；停用品种在右侧竖排折叠，点击启用并展开，再点击品种标题停用。普通节点整体点击切换完成状态，无内嵌操作按钮。询价节点右侧连接文本框，记录按账号及上海自然日保存在本地；旧版每日记录自动补空 notes，不丢失已有进度。
- 编辑模式下直接拖动节点调整位置，点击节点打开右侧编辑面板；拖动不改变业务顺序及分支关系，无拖动按钮。偏移量相对自动布局保存于配置的可选 offset 字段，分支展开后仍跟随布局；面板数字字段提供键盘与单指操作替代。询价文本框可通过可选 inquiry 字段配置，既有询价节点兼容旧配置。分支及品种的编辑预览不修改当日激活状态，取消编辑不保存配置；保存失败保留草稿。
...
- header 只新增日期和编辑模式；内容区无小标题、完成统计、定位/刷新/缩放等工具栏。保存、增加、删除和布局恢复仅在节点编辑面板。
