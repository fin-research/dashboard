...
- 交易流程使用 Svelte Flow，顶部拆借、逆回购、交易所回购为分类项，不作为节点、不连线，保留品种浅色底框，文字与相邻归档按钮整体居中。折叠后在右侧形成纵向排列的竖排紧凑标签，点击展开；剩余品种重排填满工作区，过渡为 240ms，减少动效时关闭。询价分别位于各品种下方，点击展开可编辑表格，不参与完成状态和待办提醒；按账号、上海自然日和原节点 ID 保存记录，旧记录兼容。
...
- 左侧时间线使用可见节点的显式起始时间作为锚点，时间游标每秒随上海时钟在锚点间插值；重复时间合并，非递增时点不作为坐标锚点，不给无时间节点创造期限。节点按正常字号与自然高度渲染，禁止 fitView 压缩整图；较长流程通过内容区自然滚动，窄屏仅画布局部横向滚动。节点最大宽度 460px，列内居中；相邻节点保留至少 76px 垂直间距。同一目标的入边共用目标上方的水平汇合线，节点实测高度变动后重排并更新连线。
