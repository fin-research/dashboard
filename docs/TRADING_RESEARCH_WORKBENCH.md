...
- 交易流程使用 Svelte Flow，顶部拆借、逆回购、交易所回购为分类项，不作为节点、不连线，保留品种浅色底框，文字居中，不设归档按钮。右侧始终显示三条流程的竖排标签，点击切换展开/折叠；剩余品种重排填满工作区，过渡为 240ms，减少动效时关闭。询价分别位于各品种下方，点击展开可编辑表格，不参与完成状态和待办提醒；按账号、上海自然日和原节点 ID 保存记录，旧记录兼容。
...
- 节点按显式归属展示，不按同名自动合并；衡泰补单审批分别属于各流程，进度与编辑彼此独立。共通节点独立于三条流程，品种折叠不隐藏共通节点；有流程节点介于共通节点之间时不添加共通节点直连。条件分支的子节点在右侧独立列展开，主流程继续向下，展开不把后续主流程节点向下挤移。
