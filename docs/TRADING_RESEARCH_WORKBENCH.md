...
交易研究工作台包含八个侧栏页面：总览、交易管理、市场热点、政策跟踪、跟踪点评、研究辅助、二级池周报和融资择时模型。交易管理在页头提供交易记录、交易流程两个标签页。授信工作台独立为首页一级入口，提供授信一览表、授信日历、授信周报、授信助手。授信报表已经接入 Neon `credit` schema：业务人员每周在本地解析 Excel 并按报告日期写入数据库，浏览器通过同源 `/api/credit` 和 Hyperdrive 读取。研究辅助的首次全历史回填由受控本地命令完成；此后 dashboard Worker 每日增量读取 Choice EDB 与 DM 资金利率历史并写入 Neon `public.edb`，浏览器只通过同源 `/api/economic-indicators` 读取；交易管理仍使用仓库内冻结数据；交易流程节点配置保存在 D1，每日进度仅保存在浏览器本地；二级池周报与融资择时复用独立路由的生产页面组件和既有数据链路。
...
- D1 `trading_workflow_config` 只存单份共享配置及版本；migration `1015_trading_workflow_config.sql` 播种全部节点；`1016_trading_workflow_graph.sql` 原位将既有数组转换为 `{flows,nodes}` JSON 文档，保留 ID、备注、图标、偏移和询价配置，生成显式连接并增加版本。原 `nodes` 列承载该配置文档，重复执行转换不覆盖新配置；读取兼容尚未迁移的旧数组，保存只接受新契约。只有配置保存会写数据库，不存每日日期、完成状态、分支选择、提醒记录、用户任务或审批审计。
