...
- GDP 统一为已有 `EMM00000012` 不变价同比，Quant 按真实发布日期对齐。不得将现价同比填入此 ID。2019 年 M2/社融共 24 条上游 `PUBLISHDATE` 为空，现按人民银行原始公告日期修复（证据逐条保存在 `economic-release-evidence.ts`），只更新 NULL 日期，不改变现有值。日期是统计期首次公布日，不能证明当前修订值当日已可得，仍不是完整历史时点版本。
...
- R2 `fund-reports/YYYY-MM-DD.html` 只解析有限的静态数字，不执行 JavaScript。`company_report.static_gap_1m` 是日报的静态缺口，保留源字段名和来源，与资金计划表的 `company.funding_gap_1m` 分开存储；Quant 现统一使用日报 `static_gap_1m` 作为资金缺口输入，旧资金计划缺口不参与拼接，报告隐藏其数值与状态。`margin_scale_previous_trade` 明确保留“截至上一交易日”标签，观察索引为报告日；未经交易日历转换不能当作报告日公司余额。日报没有主体利差，原本地历史单独保留。
...
维护时可使用 Workflow 参数 `quantOnly: true` 只刷新新增输入，避免重放已有 EDB 查询。`partial` 表示仍有失败来源，不等于完整模型覆盖。Choice 的 `BondTradingStatistics` 需要账户权限；拒绝访问时仅记录失败，不能用其他债市成交口径代替。生产同一账号其他债券 CTR 成功而该报表返回 `10001003`，需为 office `CHOICE_AUTH` 账号开通 `BondTradingStatistics` 专题授权；恢复后受控重跑二级成交窗口，勿重刷 EDB。
...
发布日期定向修复：`node --env-file-if-exists=.env.local scripts/repair-edb-release-dates.ts` 默认盘点，`--apply` 只更新上述 24 个键的 NULL 日期；幂等重跑，有冲突则整批回滚，不调用 EDB。
