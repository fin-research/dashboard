...
- `public.edb` 保存资金利率与宏观指标。资金面与 Choice ID 对应见 `src/lib/server/quant-input-contract.ts`；国开债 3Y/10Y 为 `E1707108`/`E1707114`，AAA 企业债 3Y 为 `E1000413`。DR007/R007、SHIBOR O/N/1W/3M 走 DM history；SHIBOR 读取 `lastPrice`，不使用 DR/R 的 `weightedYield`。O/N 的 DM 参数为 `ShiborO/N`，3M 的 EDB ID 为 `E1300079`；O/N、1W 的内部存储代码为 `DM_SHIBOR_ON`/`DM_SHIBOR_1W`，不是虚构的 Choice ID。
...
3. 发行利差每次最多取 64 只尚无利差的券；失效/空值代码登记为待处理并排除自动付费重试，避免永久卡住下一批。
