...
- 融资项目负责人、任务执行人、周报生成人直接保存 Auth0 用户 ID；SOP 默认角色保存 Auth0 角色 ID。姓名和角色从请求内 Auth0 目录解析，不按邮箱或姓名自动绑定。
...
- 保存提交角色 ID、权限代码列表及版本，严格限制请求体和字段。服务端重新确认 Auth0 角色存在，在数据库事务中锁定角色、比对版本并保存；并发冲突返回 409，不能覆盖他人的配置。
...
- `GET /auth/session` 只返回现有公开身份 DTO，不返回权限、认证时间、密钥或内部 metadata。
...
- 全站个人入口显示图标、姓名和部门。受保护页面复用服务端 layout 的最小 `account` 展示 DTO；公共页面按需读取 `/auth/session` 的 `account` 字段，原有 `user` DTO 保持不变。只返回姓名及规范化的部门字符串，不透传用户 metadata、角色或凭据；匿名不查询 Auth0。
- 个人管理页显示姓名和只读部门，继续提供既有姓名、邮箱、密码及行情偏好操作。保存姓名后刷新 layout 的账号展示。
