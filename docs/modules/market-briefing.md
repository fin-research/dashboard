...
- 浏览器携带同源 Access 会话直接请求 `/data/*` 的单一上游映射 REST 资源，在 `src/market-report-resources.ts` 完成筛选、合并和口径换算；市场数据不经过 Dashboard Worker，也不使用 Data GraphQL 做报告聚合。单一资源请求失败不得阻断整份报告，依赖该资源的视觉模块与文字段落明确显示“数据缺失”，其余模块保持正常数据。
...
行情原始资源由 Data Worker 校验 Cloudflare Access JWT 的签名、issuer、audience、有效期和本站用户邮箱，不为每个行情请求调用 Dashboard 授权入口或 Auth0 Management API。页面权限、定稿读取／保存及 AI 生成仍执行各自应用权限。旧 `/api/market-resources/*` 仅兼容已打开的页面，验证 Access 登录及资源／参数白名单后通过私有 DATA binding 读取。
