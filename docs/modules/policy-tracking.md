...
入口：`/trading-research/policy-tracking`。公共规则见 [文档分流](../../AGENTS.md#context-routing)；仅在任务涉及本模块时读取。
...
- 点评撰写与人工编辑已归集到[跟踪点评](tracking-commentary.md)，政策卡片保留跳转；`POST /api/policies/{id}/commentary` 兼容调用统一研报原文选材管线。
