...
- `credit.institution_client` 是唯一的静态一对多关联表，不保存报告日，也不再另存 `client_mapping`。每个客户的收益凭证和拆借分别只能归属于一个授信主体。银行金市/资管分开的授信按品种分配，关闭某一品种的归属表示该主体该品种使用为零，不重复计算。
...
维护清单按名称更新客户和给定别名/静态归属，不删除未提及客户、旧别名或关联。改变归属需显式维护 `institution_client`；同一客户的重复品种归属会被数据库拒绝。`yield_certificate` / `interbank_lending` 是必要的品种归属开关，例如上海农商行金市承接拆借、资管承接收益凭证，关闭的品种计算为零。负债详情可编辑客户 ID；修改历史金额、日期、现金流后按需要显式刷新既有月度衍生指标。
