...
发行建议仍由已发布的Quant模型生成；新包直接输出“尽快发行／等待／暂缓发行”，旧运行的“可按资金计划发行”和“可择机等待”仅在页面映射为对应业务标签。未经人工编辑的旧模型结论正文同步改为对应的三档业务结论，不再呈现窗口净节约或等待概率；人工正文原样保留。无法识别的模型动作不猜测为三档之一。旧分位三等分、六维支持度、四品种相对中位数推荐及旧胜率不再用于当前结论。
...
整体结论与业务指标上下排列；因子贡献合为一张卡，左侧雷达按真实非零SHAP动态显示利率、信用、资金、宏观、一级、二级与日历贡献，右侧显示绝对值最大因子并保留有贡献的宏观、一级、二级代表项。缺失因子值显示“暂缺”，不填零。未来发行窗口/模型验证按3:1展示，窄屏保持顺序单列；未来窗口只展示票面及区间。窗口明细、决策录入和卖方生成仍使用标题旁操作。2026为已观察研究参考期，不标作新独立测试。
...
`0008_issuance_forecast.sql`与`0009_issuance_full_inputs.sql`由Dashboard维护，通过`pnpm financing-model:db:migrate`应用。发布只接受真实`cloudflare-workflow`、`issuance-lgb-v1/v2`来源与匹配的R2归档key；校验模型有效期、输入/标签日期、市场路径及SHAP加法与票面一致性。旧结果契约发布被拒绝。
