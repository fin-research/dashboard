...
- 日历分为额度、已用两组独立多选筛选，均默认“全部”，复用 `MultiSelectFilter`；切月和切换授信标签保留筛选。额度选项为全部、新增、到期、续作/扩额、撤销；已用选项为全部及六个分项。两组分别筛选各自事件后合并展示。
...
- 已用按业务日、机构、具体分项显示净变化：`同业拆借 · 增加1亿元`、`收益凭证 · 减少2亿元`；不显示合计变动。融资部分读取实际生效、到期、结清、关闭日期；人工部分读取 diff 日期。每条已用事件返回稳定的 `itemType`，按分项筛选不依赖解析中文文案。
