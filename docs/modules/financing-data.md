...
- `client_id` 是单客户关联，继承子表各自显式安装外键；客户主数据、别名及人工维护见 [客户与授信关联](clients.md)。新行按交易对手自动识别，人工指定优先；非空交易对手无法识别时整个导入拒绝并回滚。
...
- 线上借入资金汇总表仅导入增量：浏览器解析并校验工作簿，以身份清单核对存量后，只 POST 新增负债、对应现金流与新余额日期；advisory lock 串行化写入，核对版本改变时整批拒绝并要求重试。按稳定业务身份匹配的历史负债及其现金流全部保留，包括金额、利率、生命周期日期和人工客户关联；仅为新增负债插入现金流。历史余额按日期/品种键冲突跳过。工作簿自身分项与合计仍硬校验；历史余额与新工作簿不同只提示管理员维护，不覆盖历史。唯一匹配产品的到期日修订不产生新负债，重复身份拒绝导入。
...
- 同一 POST 接口先以 `action=plan` 核对完整身份清单（不含历史金额/现金流），返回新增来源键、已有余额键与数据版本；浏览器筛选增量后用 `action=commit` 提交。提交保留身份清单用于重新匹配，完整工作簿只留在浏览器，JSON 上限 16 MB。
- 服务端在事务 advisory lock 内重新核对版本、新增键、客户关联和余额分项，负债、现金流、余额和缺失月度指标一并提交；任何数据错误整体回滚，返回明确拒绝。请求结果未知时重新核对再导入，不盲目重放旧增量。
