...
- `scripts/financing/import-debts.mjs` 与 HTTP 接口复用同一原子 importer。保留历史金额、利率、生命周期、人工客户和旧余额；历史余额差异仍提示管理员维护，不视为覆盖历史的理由。无法识别的非空工作表、无效数值/日期及孤儿现金流拒绝导入；明确的集团可转债说明段落不属于业务记录。
