...
- `client_id` 是单客户关联，继承子表各自显式安装外键；客户主数据、别名及人工维护见 [客户与授信关联](clients.md)。新行先按交易对手识别，未命中时在导入事务内自动创建可确定名称的客户并关联；仍有歧义的对手方保持空关联并在结果中提示，不阻断其他负债。已有人工指定优先。
...
- 线上借入资金汇总表仅导入增量：浏览器解析并校验工作簿，向服务端请求现有负债身份及余额键索引，在本地筛出候选新增后只上传这些负债、对应现金流与新余额日期；不上传历史身份清单。advisory lock 串行化写入，索引版本改变时要求重试。按稳定业务身份匹配的历史负债及其现金流全部保留，包括金额、利率、生命周期日期和人工客户关联；仅为新增负债插入现金流。历史余额按日期/品种键冲突跳过。工作簿自身分项与合计仍硬校验；历史余额与新工作簿不同只提示管理员维护，不覆盖历史。唯一匹配产品的到期日修订不产生新负债，重复身份拒绝导入。
...
- 导入不保存任务状态；数据库只保存原子提交后的业务表与衍生表结果。
- 人员与角色由 Auth0 管理；旧 `authorization` 表只供历史回溯，不参与运行时授权。
...
- 借入资金汇总表在浏览器 Web Worker 解析 `.xlsx`，文件上限 10 MB；只提交 JSON，不使用 Workflow、Protobuf、Brotli 或后台任务轮询。
- 同一 POST 接口先以空的 `action=index` 请求读取现有身份索引、余额键与数据版本；浏览器本地筛选后以 `action=commit` 仅提交候选新增。完整工作簿始终留在浏览器，JSON 上限 16 MB。
- 服务端在事务 advisory lock 内检查索引版本并按既有规则去重，自动创建新客户，再将负债、现金流、余额和缺失月度指标一并提交；数据错误整体回滚。请求结果未知时重新获取索引再导入，不盲目重放旧增量。
- `scripts/financing/import-debts.mjs` 与 HTTP 接口复用同一原子 importer。保留历史金额、利率、生命周期、人工客户和旧余额；历史余额差异仍提示管理员维护，不视为覆盖历史的理由。无法识别的非空工作表、业务金额或日期无效及孤儿现金流拒绝导入；互换便利“正回购日均余额（元）”中的文字按说明保留，跳过数值校验并在导入成功后提示。发行失败且无金额的收益凭证、债券发行及兑付说明段落、集团可转债说明段落不属于业务记录，跳过数量在导入后提示。
...
- 数据后台的借入资金汇总表导入只对具有 `financing.data:import` 权限的用户显示，置于财务指标模块之前，并作为该页唯一最高优先级操作。
...
- 文件选定后显示名称和大小；按钮分别呈现解析中、核对中、提交中，结果笔数和错误使用全局消息层，不展示虚构百分比或任务历史。
- 不轮询后台任务；错误保留可修正提示和已选择文件。连接故障可能造成结果未知，明确提示重新核对后重试，不声明未提交。
