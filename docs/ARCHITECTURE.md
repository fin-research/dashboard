...
- `src/lib/ai-client.svelte.ts` 是浏览器发起 AI 请求的唯一入口，统一解析 SSE、取消请求并在当前标签页内保留任务 summary 与完整终态结果；根 layout 挂载 `AiPanel.svelte`，在所有页面提供固定 AI 入口、任务列表与可进入的详情面板，并统一发布生成完成通知。业务页面只提交任务标题、路由、请求参数与终态 Schema，不自行解析流。
